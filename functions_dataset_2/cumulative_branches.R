cumulative_branches <- function(total) {
  ...
  total <- total %>%
    mutate(
      "cBins" = cut(Collapse, breaks = seq(0, 30, 2), include.lowest = TRUE),
      "Bins" = cut(Formation, breaks = seq(0, 30, 2), include.lowest = TRUE)
    )
  ...
  temp <- total %>%
    #filter(Collapse < 24) %>%
    group_by(Treatment, Location, Branchtype, CellSum, Bins) %>%
    summarise("Formed" = n()) %>%
    ungroup()
  ...
  temp1 <- total %>%
    filter(Collapse < 30) %>%
    group_by(Treatment, Location, Branchtype, CellSum, cBins) %>%
  ...
  binned_branches <- full_join(temp, temp1,
    ...
    complete(Bins, nesting(Treatment, Location, Branchtype))
  ...
  Cumulative <- binned_branches %>%
    ...
    arrange(Treatment, Location, Branchtype, Bins) 
  ...
  Cumulative <- Cumulative %>%
    group_by(Treatment, Location, Branchtype) %>%
    # accumulate 
